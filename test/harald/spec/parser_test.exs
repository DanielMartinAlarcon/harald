defmodule Harald.Spec.ParserTest do
  ...
  describe "command_groups" do
    test "command" do
      ...
      assert 6 = length(actual_ast_maps)
      ...
      assert_ast_lists(expected_deserializers, actual_deserializers)
      ...
      assert_ast_lists(expected_serializers, actual_serializers)
  ...
  defp assert_ast_lists(asts_1, asts_2) do
    assert length(asts_1) == length(asts_2)

    Enum.reduce(asts_1, asts_2, fn
      [ast_1], [[ast_2] | asts_2] ->
        assert format_ast(ast_1) == format_ast(ast_2)
        asts_2
