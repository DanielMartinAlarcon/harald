defmodule Harald.Parser do
  ...
  defp reduce_spec({:mappings, mappings}, context) do
    Enum.reduce(mappings, context, fn {name, mappings}, context ->
      Enum.reduce(mappings, context, fn
        {unparsed, parsed}, context ->
          deserializer =
            quote context: Elixir do
              def deserialize({unquote(name), unquote(unparsed)}), do: unquote(parsed)
            end

          serializer =
            quote context: Elixir do
              def serialize({unquote(name), unquote(parsed)}), do: unquote(unparsed)
            end

          context =
            context
            |> Map.put(:target, Context.target(:mapping, name, [], nil))
            |> update_in([:target, :deserializers], &[deserializer | &1])
            |> update_in([:target, :serializers], &[serializer | &1])
  ...
  def generate_helpers(context) do
    command_name_funs =
      Enum.reduce(context.types.opcode.mapping, [], fn {opcode, command_name}, acc ->
