defmodule Harald.Spec do
  ...
  defmacro ast(version) when is_atom(version) do
    context =
      version
      |> definition()
      |> Parser.parse_spec()
    ...
    [
      ...
      Parser.generate_helpers(context) ++
      Enum.reduce(context.ast_maps, [], fn ast_map, acc ->
        ast_map.serializers ++ ast_map.deserializers ++ acc
      end)
  end
      ...
      generic_access_profile: [
        %{
          ...
          parameters: [
            ...
            %{name: "AD Type", value: 32, hidden: true},
            %{name: "UUID", size: 32},
            %{name: "data", type: :binary, size: :remaining}
          ]
        }
      ]
    ]
